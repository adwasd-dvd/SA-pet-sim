...
- `npm run profile:pet-field:classic-core` writes the first lazy pet field pack: `public/data/profiles/classic-core/pet-field-animations.json` plus `packs/pet-field-priority.*`. As of 2026-05-19 it intentionally includes 9 priority sprites (乌力系, 布伊, 加比奥, 凯比, 猪雀, 奥卡洛斯, 黑乌力), 2135 field bitmap frames, and a 2.43 MiB indexed PNG instead of the 21.7 MiB all-core field pack. Frontend map/PET STATUS/pet cards now lazy-load this pack and fall back to static ImgNo until it arrives.
