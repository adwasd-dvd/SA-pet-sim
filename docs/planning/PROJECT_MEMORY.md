...
- Automatic walking encounters and the visible encounter/capture UI are disabled per user direction. NPC dialogue can now explicitly create a source-grounded `startBattle` VM state from current-map `encount.txt` and continue with `battleAction` attack/release commands in the same dialog, but the old auto capture panel remains hidden until the battle/capture loop is redesigned around original game semantics.
