...
- `script-vm-001` has started: deterministic NPC replies now record `npcVmEvents` for whitelisted actions such as `say`, `quest`, `shop`, `warp`, `heal`, `save`, `debug`, and unsupported future actions; dialog debug returns the current NPC's recent `vmTrace`, and SAAC-like save JSON carries recent VM events for inspection.
