...
- Classic-core closure manifest started 2026-05-13: `npm run closure:classic-core` now writes `docs/planning/CLASSIC_CORE_CLOSURE_MANIFEST.md` and `docs/planning/classic-core-closure-manifest.json`. The first draft groups classic-core and staged advanced lines, reports generated/source-only floors, NPCs, scripts, shop items, encounters, enemy tempNos, pet bitmap frame coverage, package-impact estimates, and closed/hidden warp candidates. One-hop transit maps are kept as decision candidates rather than automatically enabled, so the closure can drive slimming instead of recreating a broad full-dev map bundle.
...
- Classic-core package report added 2026-05-13: `npm run report:classic-core` writes `docs/planning/CLASSIC_CORE_PACKAGE_REPORT.md` and `docs/planning/classic-core-package-report.json`. Current estimate: full-dev current 260 floors versus classic-core 131 floors, 63 profile-closed exits, no missing classic-core map/DAT assets, about 15.02 MiB raw estimated saving. Shared assets remain 32.44 MiB raw because the client tile atlas and data tables are still monolithic; the next size win needs resource-pack splitting/lazy loading rather than more map pruning.
