...
- `script-vm-001` has started: deterministic NPC replies now record `npcVmEvents` for whitelisted actions such as `say`, `quest`, `shop`, `window`, `warp`, `heal`, `save`, `setFlag`, `give`, `take`, `startBattle`, `debug`, and unsupported future actions; dialog debug returns `actions`, VM `allowedActions`, global `supportedActions`, and the current NPC's recent `vmTrace`, and SAAC-like save JSON carries recent VM events for inspection. Stateful `setFlag/give/take/startBattle` actions now run through `runNpcVmAction`, mutate flags/stone/exp/inventory/encounter state inside the NPC VM executor, and mark trace detail with `executor: npc-action-vm` plus `mutated`.
...
- Automatic walking encounters and the visible encounter/capture UI are disabled per user direction. NPC dialogue can now explicitly create a source-grounded `startBattle` VM state from current-map `encount.txt`, but the old auto capture panel remains hidden until the battle/capture loop is redesigned around original game semantics.
