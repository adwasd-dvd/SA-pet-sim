...
const sotD8Npc = WORLD.maps["11002"]?.npcs.find((npc) => npc.id === "11002-27-42-7697");
if (!sotD8Npc) throw new Error("missing Keao mountain small cave SOT D-8 checkpoint fixture");
assertEqual(sotD8Npc.name, "检查员(D-8)", "SOT D-8 keeps source NPC name");
assert(sotD8Npc.scriptEvents?.some((event) => event.type === "MESSAGE" && event.condition === "LV>0&ITEM=2582" && event.keyword === "达克尔" && event.delItems?.some((item) => Number(item.id) === 2582) && event.getItems?.some((item) => Number(item.id) === 2583)), "SOT D-8 parses source KeyWord checkpoint exchange");
assert(sotD8Npc.scriptEvents?.some((event) => event.type === "MESSAGE" && event.condition === "LV>0&ITEM=2583,LV>0&ITEM=2584,LV>0&ITEM=2585,LV>0&ITEM=2586,LV>0&ITEM=2587"), "SOT D-8 parses source already-passed MESSAGE branch");
assert(sotD8Npc.scriptEvents?.some((event) => event.type === "MESSAGE" && event.condition === "LV>0&ITEM=2575,LV>0&ITEM=2576,LV>0&ITEM=2577,LV>0&ITEM=2578,LV>0&ITEM=2579,LV>0&ITEM=2580,LV>0&ITEM=2581"), "SOT D-8 parses source too-early MESSAGE branch");
let sotD8PromptGame = await api("/api/game/new", { name: "keao-small-cave-sot-d8-prompt-runtime-test" });
sotD8PromptGame.location = { mapId: "11002", x: sotD8Npc.x + 1, y: sotD8Npc.y };
sotD8PromptGame.inventory.push({ id: 2582, name: "检查石", qty: 1 });
sotD8PromptGame = await api("/api/game/dialog", { game: sotD8PromptGame, npcId: sotD8Npc.id });
assertEqual(inventoryQty(sotD8PromptGame, 2582), 1, "SOT D-8 prompt branch does not consume seventh checkpoint stone without keyword");
assertEqual(inventoryQty(sotD8PromptGame, 2583), 0, "SOT D-8 prompt branch does not grant eighth checkpoint stone without keyword");
assert(sotD8PromptGame.dialog.messages.some((message) => /请说出旁边看板/.test(message.text || "") && /D路线/.test(message.text || "")), "SOT D-8 prompt branch asks for the source signboard answer");
assert(sotD8PromptGame.dialog.debug.vmTrace.some((event) => event.action === "window" && event.detail?.eventType === "MESSAGE" && event.detail?.condition === "LV>0&ITEM=2582" && event.detail?.keywordRequired === false && event.status === "ok"), "SOT D-8 prompt branch selects the non-keyword MESSAGE branch");
let sotD8Game = await api("/api/game/new", { name: "keao-small-cave-sot-d8-keyword-runtime-test" });
sotD8Game.location = { mapId: "11002", x: sotD8Npc.x + 1, y: sotD8Npc.y };
sotD8Game.inventory.push({ id: 2582, name: "检查石", qty: 1 });
sotD8Game = await api("/api/game/dialog", { game: sotD8Game, npcId: sotD8Npc.id, message: "达克尔" });
assertEqual(inventoryQty(sotD8Game, 2582), 0, "SOT D-8 keyword branch consumes seventh checkpoint stone");
assertEqual(inventoryQty(sotD8Game, 2583), 1, "SOT D-8 keyword branch grants eighth checkpoint stone");
assert(sotD8Game.dialog.messages.some((message) => /答对了/.test(message.text || "") && /第2检查点/.test(message.text || "") && /柯尔克的大坑道/.test(message.text || "")), "SOT D-8 keyword branch preserves source pass text");
assert(sotD8Game.dialog.debug.vmTrace.some((event) => event.action === "window" && event.detail?.eventType === "MESSAGE" && event.detail?.condition === "LV>0&ITEM=2582" && event.detail?.keywordRequired === true && event.detail?.keywordOk === true && event.status === "ok"), "SOT D-8 keyword MESSAGE branch is selected by VM");
assert(sotD8Game.dialog.debug.vmTrace.some((event) => event.action === "take" && Number(event.detail?.itemId) === 2582 && event.detail?.reason === "source-changeevent-message-delitem"), "SOT D-8 keyword branch takes item 2582 through NPC VM");
assert(sotD8Game.dialog.debug.vmTrace.some((event) => event.action === "give" && Number(event.detail?.itemId) === 2583 && event.detail?.reason === "source-changeevent-message-getitem"), "SOT D-8 keyword branch gives item 2583 through NPC VM");
assert(sotD8Game.dialog.debug.vmTrace.some((event) => event.action === "quest" && event.detail?.eventType === "MESSAGE" && event.detail?.keywordRequired === true && event.detail?.keywordOk === true && event.detail?.getItems?.some((item) => Number(item.id) === 2583) && event.detail?.delItems?.some((item) => Number(item.id) === 2582)), "SOT D-8 keyword branch records source exchange metadata");
let sotD8PassedGame = await api("/api/game/new", { name: "keao-small-cave-sot-d8-passed-runtime-test" });
sotD8PassedGame.location = { mapId: "11002", x: sotD8Npc.x + 1, y: sotD8Npc.y };
sotD8PassedGame.inventory.push({ id: 2583, name: "检查石", qty: 1 });
sotD8PassedGame = await api("/api/game/dialog", { game: sotD8PassedGame, npcId: sotD8Npc.id });
assertEqual(inventoryQty(sotD8PassedGame, 2583), 1, "SOT D-8 already-passed branch does not duplicate checkpoint stone");
assert(sotD8PassedGame.dialog.messages.some((message) => /已经通过/.test(message.text || "") && /大坑道/.test(message.text || "")), "SOT D-8 already-passed branch uses source follow-up text");
assert(sotD8PassedGame.dialog.debug.vmTrace.some((event) => event.action === "window" && event.detail?.eventType === "MESSAGE" && event.detail?.condition === "LV>0&ITEM=2583,LV>0&ITEM=2584,LV>0&ITEM=2585,LV>0&ITEM=2586,LV>0&ITEM=2587" && event.status === "ok"), "SOT D-8 already-passed MESSAGE branch is selected by VM");
let sotD8EarlyGame = await api("/api/game/new", { name: "keao-small-cave-sot-d8-too-early-runtime-test" });
sotD8EarlyGame.location = { mapId: "11002", x: sotD8Npc.x + 1, y: sotD8Npc.y };
sotD8EarlyGame.inventory.push({ id: 2581, name: "检查石", qty: 1 });
sotD8EarlyGame = await api("/api/game/dialog", { game: sotD8EarlyGame, npcId: sotD8Npc.id });
assertEqual(inventoryQty(sotD8EarlyGame, 2581), 1, "SOT D-8 too-early branch preserves sixth checkpoint stone");
assertEqual(inventoryQty(sotD8EarlyGame, 2583), 0, "SOT D-8 too-early branch does not grant eighth checkpoint stone");
assert(sotD8EarlyGame.dialog.messages.some((message) => /还太早了一点/.test(message.text || "") && /按照顺序通过/.test(message.text || "")), "SOT D-8 too-early branch uses source order text");
assert(sotD8EarlyGame.dialog.debug.vmTrace.some((event) => event.action === "window" && event.detail?.eventType === "MESSAGE" && event.detail?.condition === "LV>0&ITEM=2575,LV>0&ITEM=2576,LV>0&ITEM=2577,LV>0&ITEM=2578,LV>0&ITEM=2579,LV>0&ITEM=2580,LV>0&ITEM=2581" && event.status === "ok"), "SOT D-8 too-early MESSAGE branch is selected by VM");

const sotD9Npc = WORLD.maps["10912"]?.npcs.find((npc) => npc.id === "10912-35-4-7698");
if (!sotD9Npc) throw new Error("missing Korku mine SOT D-9 checkpoint fixture");
assertEqual(sotD9Npc.name, "检查员(D-9)", "SOT D-9 keeps source NPC name");
assert(sotD9Npc.scriptEvents?.some((event) => event.type === "MESSAGE" && event.condition === "LV>0&ITEM=2583" && event.keyword === "查罕吉鲁兄弟" && event.delItems?.some((item) => Number(item.id) === 2583) && event.getItems?.some((item) => Number(item.id) === 2584)), "SOT D-9 parses source KeyWord checkpoint exchange");
assert(sotD9Npc.scriptEvents?.some((event) => event.type === "MESSAGE" && event.condition === "LV>0&ITEM=2584,LV>0&ITEM=2585,LV>0&ITEM=2586,LV>0&ITEM=2587"), "SOT D-9 parses source already-passed MESSAGE branch");
assert(sotD9Npc.scriptEvents?.some((event) => event.type === "MESSAGE" && event.condition === "LV>0&ITEM=2575,LV>0&ITEM=2576,LV>0&ITEM=2577,LV>0&ITEM=2578,LV>0&ITEM=2579,LV>0&ITEM=2580,LV>0&ITEM=2581,LV>0&ITEM=2582"), "SOT D-9 parses source too-early MESSAGE branch");
let sotD9PromptGame = await api("/api/game/new", { name: "korku-mine-sot-d9-prompt-runtime-test" });
sotD9PromptGame.location = { mapId: "10912", x: sotD9Npc.x + 1, y: sotD9Npc.y };
sotD9PromptGame.inventory.push({ id: 2583, name: "检查石", qty: 1 });
sotD9PromptGame = await api("/api/game/dialog", { game: sotD9PromptGame, npcId: sotD9Npc.id });
assertEqual(inventoryQty(sotD9PromptGame, 2583), 1, "SOT D-9 prompt branch does not consume eighth checkpoint stone without keyword");
assertEqual(inventoryQty(sotD9PromptGame, 2584), 0, "SOT D-9 prompt branch does not grant ninth checkpoint stone without keyword");
assert(sotD9PromptGame.dialog.messages.some((message) => /请说出旁边看板/.test(message.text || "") && /D路线/.test(message.text || "")), "SOT D-9 prompt branch asks for the source signboard answer");
assert(sotD9PromptGame.dialog.debug.vmTrace.some((event) => event.action === "window" && event.detail?.eventType === "MESSAGE" && event.detail?.condition === "LV>0&ITEM=2583" && event.detail?.keywordRequired === false && event.status === "ok"), "SOT D-9 prompt branch selects the non-keyword MESSAGE branch");
let sotD9Game = await api("/api/game/new", { name: "korku-mine-sot-d9-keyword-runtime-test" });
sotD9Game.location = { mapId: "10912", x: sotD9Npc.x + 1, y: sotD9Npc.y };
sotD9Game.inventory.push({ id: 2583, name: "检查石", qty: 1 });
sotD9Game = await api("/api/game/dialog", { game: sotD9Game, npcId: sotD9Npc.id, message: "查罕吉鲁兄弟" });
assertEqual(inventoryQty(sotD9Game, 2583), 0, "SOT D-9 keyword branch consumes eighth checkpoint stone");
assertEqual(inventoryQty(sotD9Game, 2584), 1, "SOT D-9 keyword branch grants ninth checkpoint stone");
assert(sotD9Game.dialog.messages.some((message) => /答对了/.test(message.text || "") && /第9检查点/.test(message.text || "") && /阿布的洞窟/.test(message.text || "")), "SOT D-9 keyword branch reports source pass text");
assert(sotD9Game.dialog.debug.vmTrace.some((event) => event.action === "window" && event.detail?.eventType === "MESSAGE" && event.detail?.condition === "LV>0&ITEM=2583" && event.detail?.keywordRequired === true && event.detail?.keywordOk === true && event.status === "ok"), "SOT D-9 keyword MESSAGE branch is selected by VM");
assert(sotD9Game.dialog.debug.vmTrace.some((event) => event.action === "take" && Number(event.detail?.itemId) === 2583 && event.detail?.reason === "source-changeevent-message-delitem"), "SOT D-9 keyword branch takes item 2583 through NPC VM");
assert(sotD9Game.dialog.debug.vmTrace.some((event) => event.action === "give" && Number(event.detail?.itemId) === 2584 && event.detail?.reason === "source-changeevent-message-getitem"), "SOT D-9 keyword branch gives item 2584 through NPC VM");
assert(sotD9Game.dialog.debug.vmTrace.some((event) => event.action === "quest" && event.detail?.eventType === "MESSAGE" && event.detail?.keywordRequired === true && event.detail?.keywordOk === true && event.detail?.getItems?.some((item) => Number(item.id) === 2584) && event.detail?.delItems?.some((item) => Number(item.id) === 2583)), "SOT D-9 keyword branch records source exchange metadata");
let sotD9PassedGame = await api("/api/game/new", { name: "korku-mine-sot-d9-passed-runtime-test" });
sotD9PassedGame.location = { mapId: "10912", x: sotD9Npc.x + 1, y: sotD9Npc.y };
sotD9PassedGame.inventory.push({ id: 2584, name: "检查石", qty: 1 });
sotD9PassedGame = await api("/api/game/dialog", { game: sotD9PassedGame, npcId: sotD9Npc.id });
assertEqual(inventoryQty(sotD9PassedGame, 2584), 1, "SOT D-9 already-passed branch does not duplicate checkpoint stone");
assert(sotD9PassedGame.dialog.messages.some((message) => /已经通过/.test(message.text || "") && /阿布的洞窟/.test(message.text || "")), "SOT D-9 already-passed branch uses source follow-up text");
assert(sotD9PassedGame.dialog.debug.vmTrace.some((event) => event.action === "window" && event.detail?.eventType === "MESSAGE" && event.detail?.condition === "LV>0&ITEM=2584,LV>0&ITEM=2585,LV>0&ITEM=2586,LV>0&ITEM=2587" && event.status === "ok"), "SOT D-9 already-passed MESSAGE branch is selected by VM");
let sotD9EarlyGame = await api("/api/game/new", { name: "korku-mine-sot-d9-too-early-runtime-test" });
sotD9EarlyGame.location = { mapId: "10912", x: sotD9Npc.x + 1, y: sotD9Npc.y };
sotD9EarlyGame.inventory.push({ id: 2582, name: "检查石", qty: 1 });
sotD9EarlyGame = await api("/api/game/dialog", { game: sotD9EarlyGame, npcId: sotD9Npc.id });
assertEqual(inventoryQty(sotD9EarlyGame, 2582), 1, "SOT D-9 too-early branch preserves seventh checkpoint stone");
assertEqual(inventoryQty(sotD9EarlyGame, 2584), 0, "SOT D-9 too-early branch does not grant ninth checkpoint stone");
assert(sotD9EarlyGame.dialog.messages.some((message) => /还太早了一点/.test(message.text || "") && /按照顺序通过/.test(message.text || "")), "SOT D-9 too-early branch uses source order text");
assert(sotD9EarlyGame.dialog.debug.vmTrace.some((event) => event.action === "window" && event.detail?.eventType === "MESSAGE" && event.detail?.condition === "LV>0&ITEM=2575,LV>0&ITEM=2576,LV>0&ITEM=2577,LV>0&ITEM=2578,LV>0&ITEM=2579,LV>0&ITEM=2580,LV>0&ITEM=2581,LV>0&ITEM=2582" && event.status === "ok"), "SOT D-9 too-early MESSAGE branch is selected by VM");

const sotD10Npc = WORLD.maps["10006"]?.npcs.find((npc) => npc.id === "10006-8-86-7699");
if (!sotD10Npc) throw new Error("missing Abu cave SOT D-10 checkpoint fixture");
assertEqual(sotD10Npc.name, "检查员(D-10)", "SOT D-10 keeps source NPC name");
assert(sotD10Npc.scriptEvents?.some((event) => event.type === "MESSAGE" && event.condition === "LV>0&ITEM=2584" && event.keyword === "邦洛洛克斯" && event.delItems?.some((item) => Number(item.id) === 2584) && event.getItems?.some((item) => Number(item.id) === 2585)), "SOT D-10 parses source KeyWord checkpoint exchange");
assert(sotD10Npc.scriptEvents?.some((event) => event.type === "MESSAGE" && event.condition === "LV>0&ITEM=2585,LV>0&ITEM=2586,LV>0&ITEM=2587"), "SOT D-10 parses source already-passed MESSAGE branch");
assert(sotD10Npc.scriptEvents?.some((event) => event.type === "MESSAGE" && event.condition === "LV>0&ITEM=2575,LV>0&ITEM=2576,LV>0&ITEM=2577,LV>0&ITEM=2578,LV>0&ITEM=2579,LV>0&ITEM=2580,LV>0&ITEM=2581,LV>0&ITEM=2582,LV>0&ITEM=2583"), "SOT D-10 parses source too-early MESSAGE branch");
let sotD10PromptGame = await api("/api/game/new", { name: "abu-cave-sot-d10-prompt-runtime-test" });
sotD10PromptGame.location = { mapId: "10006", x: sotD10Npc.x + 1, y: sotD10Npc.y };
sotD10PromptGame.inventory.push({ id: 2584, name: "检查石", qty: 1 });
sotD10PromptGame = await api("/api/game/dialog", { game: sotD10PromptGame, npcId: sotD10Npc.id });
assertEqual(inventoryQty(sotD10PromptGame, 2584), 1, "SOT D-10 prompt branch does not consume ninth checkpoint stone without keyword");
assertEqual(inventoryQty(sotD10PromptGame, 2585), 0, "SOT D-10 prompt branch does not grant tenth checkpoint stone without keyword");
assert(sotD10PromptGame.dialog.messages.some((message) => /请说出旁边看板/.test(message.text || "") && /D路线/.test(message.text || "")), "SOT D-10 prompt branch asks for the source signboard answer");
assert(sotD10PromptGame.dialog.debug.vmTrace.some((event) => event.action === "window" && event.detail?.eventType === "MESSAGE" && event.detail?.condition === "LV>0&ITEM=2584" && event.detail?.keywordRequired === false && event.status === "ok"), "SOT D-10 prompt branch selects the non-keyword MESSAGE branch");
let sotD10Game = await api("/api/game/new", { name: "abu-cave-sot-d10-keyword-runtime-test" });
sotD10Game.location = { mapId: "10006", x: sotD10Npc.x + 1, y: sotD10Npc.y };
sotD10Game.inventory.push({ id: 2584, name: "检查石", qty: 1 });
sotD10Game = await api("/api/game/dialog", { game: sotD10Game, npcId: sotD10Npc.id, message: "邦洛洛克斯" });
assertEqual(inventoryQty(sotD10Game, 2584), 0, "SOT D-10 keyword branch consumes ninth checkpoint stone");
assertEqual(inventoryQty(sotD10Game, 2585), 1, "SOT D-10 keyword branch grants tenth checkpoint stone");
assert(sotD10Game.dialog.messages.some((message) => /答对了/.test(message.text || "") && /第10检查点/.test(message.text || "") && /盗贼的洞窟/.test(message.text || "")), "SOT D-10 keyword branch reports source pass text");
assert(sotD10Game.dialog.debug.vmTrace.some((event) => event.action === "window" && event.detail?.eventType === "MESSAGE" && event.detail?.condition === "LV>0&ITEM=2584" && event.detail?.keywordRequired === true && event.detail?.keywordOk === true && event.status === "ok"), "SOT D-10 keyword MESSAGE branch is selected by VM");
assert(sotD10Game.dialog.debug.vmTrace.some((event) => event.action === "take" && Number(event.detail?.itemId) === 2584 && event.detail?.reason === "source-changeevent-message-delitem"), "SOT D-10 keyword branch takes item 2584 through NPC VM");
assert(sotD10Game.dialog.debug.vmTrace.some((event) => event.action === "give" && Number(event.detail?.itemId) === 2585 && event.detail?.reason === "source-changeevent-message-getitem"), "SOT D-10 keyword branch gives item 2585 through NPC VM");
assert(sotD10Game.dialog.debug.vmTrace.some((event) => event.action === "quest" && event.detail?.eventType === "MESSAGE" && event.detail?.keywordRequired === true && event.detail?.keywordOk === true && event.detail?.getItems?.some((item) => Number(item.id) === 2585) && event.detail?.delItems?.some((item) => Number(item.id) === 2584)), "SOT D-10 keyword branch records source exchange metadata");
let sotD10PassedGame = await api("/api/game/new", { name: "abu-cave-sot-d10-passed-runtime-test" });
sotD10PassedGame.location = { mapId: "10006", x: sotD10Npc.x + 1, y: sotD10Npc.y };
sotD10PassedGame.inventory.push({ id: 2585, name: "检查石", qty: 1 });
sotD10PassedGame = await api("/api/game/dialog", { game: sotD10PassedGame, npcId: sotD10Npc.id });
assertEqual(inventoryQty(sotD10PassedGame, 2585), 1, "SOT D-10 already-passed branch does not duplicate checkpoint stone");
assert(sotD10PassedGame.dialog.messages.some((message) => /已经来过/.test(message.text || "") && /盗贼的洞窟/.test(message.text || "")), "SOT D-10 already-passed branch uses source follow-up text");
assert(sotD10PassedGame.dialog.debug.vmTrace.some((event) => event.action === "window" && event.detail?.eventType === "MESSAGE" && event.detail?.condition === "LV>0&ITEM=2585,LV>0&ITEM=2586,LV>0&ITEM=2587" && event.status === "ok"), "SOT D-10 already-passed MESSAGE branch is selected by VM");
let sotD10EarlyGame = await api("/api/game/new", { name: "abu-cave-sot-d10-too-early-runtime-test" });
sotD10EarlyGame.location = { mapId: "10006", x: sotD10Npc.x + 1, y: sotD10Npc.y };
sotD10EarlyGame.inventory.push({ id: 2583, name: "检查石", qty: 1 });
sotD10EarlyGame = await api("/api/game/dialog", { game: sotD10EarlyGame, npcId: sotD10Npc.id });
assertEqual(inventoryQty(sotD10EarlyGame, 2583), 1, "SOT D-10 too-early branch preserves eighth checkpoint stone");
assertEqual(inventoryQty(sotD10EarlyGame, 2585), 0, "SOT D-10 too-early branch does not grant tenth checkpoint stone");
assert(sotD10EarlyGame.dialog.messages.some((message) => /还太早了一点/.test(message.text || "") && /按照顺序通过/.test(message.text || "")), "SOT D-10 too-early branch uses source order text");
assert(sotD10EarlyGame.dialog.debug.vmTrace.some((event) => event.action === "window" && event.detail?.eventType === "MESSAGE" && event.detail?.condition === "LV>0&ITEM=2575,LV>0&ITEM=2576,LV>0&ITEM=2577,LV>0&ITEM=2578,LV>0&ITEM=2579,LV>0&ITEM=2580,LV>0&ITEM=2581,LV>0&ITEM=2582,LV>0&ITEM=2583" && event.status === "ok"), "SOT D-10 too-early MESSAGE branch is selected by VM");

const sotD11Npc = WORLD.maps["10703"]?.npcs.find((npc) => npc.id === "10703-58-26-7700");
if (!sotD11Npc) throw new Error("missing thieves cave SOT D-11 checkpoint fixture");
assertEqual(sotD11Npc.name, "检查员(D-11)", "SOT D-11 keeps source NPC name");
assert(sotD11Npc.scriptEvents?.some((event) => event.type === "MESSAGE" && event.condition === "LV>0&ITEM=2585" && event.keyword === "携带方便的三明治" && event.delItems?.some((item) => Number(item.id) === 2585) && event.getItems?.some((item) => Number(item.id) === 2586)), "SOT D-11 parses source KeyWord checkpoint exchange");
assert(sotD11Npc.scriptEvents?.some((event) => event.type === "MESSAGE" && event.condition === "LV>0&ITEM=2586,LV>0&ITEM=2587"), "SOT D-11 parses source already-passed MESSAGE branch");
assert(sotD11Npc.scriptEvents?.some((event) => event.type === "MESSAGE" && event.condition === "LV>0&ITEM=2575,LV>0&ITEM=2576,LV>0&ITEM=2577,LV>0&ITEM=2578,LV>0&ITEM=2579,LV>0&ITEM=2580,LV>0&ITEM=2581,LV>0&ITEM=2582,LV>0&ITEM=2583,LV>0&ITEM=2584"), "SOT D-11 parses source too-early MESSAGE branch");
let sotD11PromptGame = await api("/api/game/new", { name: "thieves-cave-sot-d11-prompt-runtime-test" });
sotD11PromptGame.location = { mapId: "10703", x: sotD11Npc.x + 1, y: sotD11Npc.y };
sotD11PromptGame.inventory.push({ id: 2585, name: "检查石", qty: 1 });
sotD11PromptGame = await api("/api/game/dialog", { game: sotD11PromptGame, npcId: sotD11Npc.id });
assertEqual(inventoryQty(sotD11PromptGame, 2585), 1, "SOT D-11 prompt branch does not consume tenth checkpoint stone without keyword");
assertEqual(inventoryQty(sotD11PromptGame, 2586), 0, "SOT D-11 prompt branch does not grant eleventh checkpoint stone without keyword");
assert(sotD11PromptGame.dialog.messages.some((message) => /请说出旁边看板/.test(message.text || "") && /D路线/.test(message.text || "")), "SOT D-11 prompt branch asks for the source signboard answer");
assert(sotD11PromptGame.dialog.debug.vmTrace.some((event) => event.action === "window" && event.detail?.eventType === "MESSAGE" && event.detail?.condition === "LV>0&ITEM=2585" && event.detail?.keywordRequired === false && event.status === "ok"), "SOT D-11 prompt branch selects the non-keyword MESSAGE branch");
let sotD11Game = await api("/api/game/new", { name: "thieves-cave-sot-d11-keyword-runtime-test" });
sotD11Game.location = { mapId: "10703", x: sotD11Npc.x + 1, y: sotD11Npc.y };
sotD11Game.inventory.push({ id: 2585, name: "检查石", qty: 1 });
sotD11Game = await api("/api/game/dialog", { game: sotD11Game, npcId: sotD11Npc.id, message: "携带方便的三明治" });
assertEqual(inventoryQty(sotD11Game, 2585), 0, "SOT D-11 keyword branch consumes tenth checkpoint stone");
assertEqual(inventoryQty(sotD11Game, 2586), 1, "SOT D-11 keyword branch grants eleventh checkpoint stone");
assert(sotD11Game.dialog.messages.some((message) => /答对了/.test(message.text || "") && /第11检查点/.test(message.text || "") && /往西走8步/.test(message.text || "") && /往北走127步/.test(message.text || "")), "SOT D-11 keyword branch reports source pass text and next checkpoint coordinates");
assert(sotD11Game.dialog.debug.vmTrace.some((event) => event.action === "window" && event.detail?.eventType === "MESSAGE" && event.detail?.condition === "LV>0&ITEM=2585" && event.detail?.keywordRequired === true && event.detail?.keywordOk === true && event.status === "ok"), "SOT D-11 keyword MESSAGE branch is selected by VM");
assert(sotD11Game.dialog.debug.vmTrace.some((event) => event.action === "take" && Number(event.detail?.itemId) === 2585 && event.detail?.reason === "source-changeevent-message-delitem"), "SOT D-11 keyword branch takes item 2585 through NPC VM");
assert(sotD11Game.dialog.debug.vmTrace.some((event) => event.action === "give" && Number(event.detail?.itemId) === 2586 && event.detail?.reason === "source-changeevent-message-getitem"), "SOT D-11 keyword branch gives item 2586 through NPC VM");
assert(sotD11Game.dialog.debug.vmTrace.some((event) => event.action === "quest" && event.detail?.eventType === "MESSAGE" && event.detail?.keywordRequired === true && event.detail?.keywordOk === true && event.detail?.getItems?.some((item) => Number(item.id) === 2586) && event.detail?.delItems?.some((item) => Number(item.id) === 2585)), "SOT D-11 keyword branch records source exchange metadata");
let sotD11PassedGame = await api("/api/game/new", { name: "thieves-cave-sot-d11-passed-runtime-test" });
sotD11PassedGame.location = { mapId: "10703", x: sotD11Npc.x + 1, y: sotD11Npc.y };
sotD11PassedGame.inventory.push({ id: 2586, name: "检查石", qty: 1 });
sotD11PassedGame = await api("/api/game/dialog", { game: sotD11PassedGame, npcId: sotD11Npc.id });
assertEqual(inventoryQty(sotD11PassedGame, 2586), 1, "SOT D-11 already-passed branch does not duplicate checkpoint stone");
assert(sotD11PassedGame.dialog.messages.some((message) => /已经来过/.test(message.text || "") && /玛丽娜丝北方出口/.test(message.text || "") && /往北走127步/.test(message.text || "")), "SOT D-11 already-passed branch uses source follow-up text");
assert(sotD11PassedGame.dialog.debug.vmTrace.some((event) => event.action === "window" && event.detail?.eventType === "MESSAGE" && event.detail?.condition === "LV>0&ITEM=2586,LV>0&ITEM=2587" && event.status === "ok"), "SOT D-11 already-passed MESSAGE branch is selected by VM");
let sotD11EarlyGame = await api("/api/game/new", { name: "thieves-cave-sot-d11-too-early-runtime-test" });
sotD11EarlyGame.location = { mapId: "10703", x: sotD11Npc.x + 1, y: sotD11Npc.y };
sotD11EarlyGame.inventory.push({ id: 2584, name: "检查石", qty: 1 });
sotD11EarlyGame = await api("/api/game/dialog", { game: sotD11EarlyGame, npcId: sotD11Npc.id });
assertEqual(inventoryQty(sotD11EarlyGame, 2584), 1, "SOT D-11 too-early branch preserves ninth checkpoint stone");
assertEqual(inventoryQty(sotD11EarlyGame, 2586), 0, "SOT D-11 too-early branch does not grant eleventh checkpoint stone");
assert(sotD11EarlyGame.dialog.messages.some((message) => /还太早了一点/.test(message.text || "") && /按照顺序通过/.test(message.text || "")), "SOT D-11 too-early branch uses source order text");
assert(sotD11EarlyGame.dialog.debug.vmTrace.some((event) => event.action === "window" && event.detail?.eventType === "MESSAGE" && event.detail?.condition === "LV>0&ITEM=2575,LV>0&ITEM=2576,LV>0&ITEM=2577,LV>0&ITEM=2578,LV>0&ITEM=2579,LV>0&ITEM=2580,LV>0&ITEM=2581,LV>0&ITEM=2582,LV>0&ITEM=2583,LV>0&ITEM=2584" && event.status === "ok"), "SOT D-11 too-early MESSAGE branch is selected by VM");

const sotD12Npc = WORLD.maps["100"]?.npcs.find((npc) => npc.id === "100-74-489-7701");
if (!sotD12Npc) throw new Error("missing Sainasu SOT D-12 checkpoint fixture");
assertEqual(sotD12Npc.name, "检查员(D-12)", "SOT D-12 keeps source NPC name");
assert(sotD12Npc.scriptEvents?.some((event) => event.type === "MESSAGE" && event.condition === "LV>0&ITEM=2586" && event.keyword === "83岁" && event.delItems?.some((item) => Number(item.id) === 2586) && event.getItems?.some((item) => Number(item.id) === 2587)), "SOT D-12 parses source KeyWord checkpoint exchange without the C-route flag");
assert(sotD12Npc.scriptEvents?.some((event) => event.type === "MESSAGE" && event.condition === "LV>0&ITEM=2587"), "SOT D-12 parses source all-checkpoints-passed MESSAGE branch");
assert(sotD12Npc.scriptEvents?.some((event) => event.type === "MESSAGE" && event.condition === "LV>0&ITEM=2547,LV>0&ITEM=2576,LV>0&ITEM=2577,LV>0&ITEM=2578,LV>0&ITEM=2579,LV>0&ITEM=2580,LV>0&ITEM=2581,LV>0&ITEM=2582,LV>0&ITEM=2583,LV>0&ITEM=2584,LV>0&ITEM=2585"), "SOT D-12 preserves the source too-early condition including item 2547");
let sotD12WrongRouteGame = await api("/api/game/new", { name: "sainasu-sot-d12-wrong-route-test" });
sotD12WrongRouteGame.location = { mapId: "100", x: sotD12Npc.x + 1, y: sotD12Npc.y };
setTestEventFlag(sotD12WrongRouteGame, 17, "end");
sotD12WrongRouteGame.inventory.push({ id: 2586, name: "检查石", qty: 1 });
sotD12WrongRouteGame = await api("/api/game/dialog", { game: sotD12WrongRouteGame, npcId: sotD12Npc.id, message: "83岁" });
assertEqual(inventoryQty(sotD12WrongRouteGame, 2586), 1, "SOT D-12 wrong-route branch preserves the C-route checkpoint stone");
assertEqual(inventoryQty(sotD12WrongRouteGame, 2587), 0, "SOT D-12 wrong-route branch grants no final checkpoint stone");
assert(sotD12WrongRouteGame.dialog.messages.some((message) => /C路线/.test(message.text || "") && /D路线/.test(message.text || "") && /原来的路线/.test(message.text || "")), "SOT D-12 wrong-route branch uses source route guidance");
let sotD12PromptGame = await api("/api/game/new", { name: "sainasu-sot-d12-prompt-runtime-test" });
sotD12PromptGame.location = { mapId: "100", x: sotD12Npc.x + 1, y: sotD12Npc.y };
sotD12PromptGame.inventory.push({ id: 2586, name: "检查石", qty: 1 });
sotD12PromptGame = await api("/api/game/dialog", { game: sotD12PromptGame, npcId: sotD12Npc.id });
assertEqual(inventoryQty(sotD12PromptGame, 2586), 1, "SOT D-12 prompt branch does not consume eleventh checkpoint stone without keyword");
assertEqual(inventoryQty(sotD12PromptGame, 2587), 0, "SOT D-12 prompt branch does not grant final checkpoint stone without keyword");
assert(sotD12PromptGame.dialog.messages.some((message) => /请说出旁边看板/.test(message.text || "") && /C路线/.test(message.text || "")), "SOT D-12 prompt branch preserves the source route wording");
let sotD12Game = await api("/api/game/new", { name: "sainasu-sot-d12-keyword-runtime-test" });
sotD12Game.location = { mapId: "100", x: sotD12Npc.x + 1, y: sotD12Npc.y };
sotD12Game.inventory.push({ id: 2586, name: "检查石", qty: 1 });
sotD12Game = await api("/api/game/dialog", { game: sotD12Game, npcId: sotD12Npc.id, message: "83岁" });
assertEqual(inventoryQty(sotD12Game, 2586), 0, "SOT D-12 keyword branch consumes eleventh checkpoint stone");
assertEqual(inventoryQty(sotD12Game, 2587), 1, "SOT D-12 keyword branch grants final checkpoint stone");
assert(sotD12Game.dialog.messages.some((message) => /答对了/.test(message.text || "") && /第12检查点/.test(message.text || "") && /玛丽娜丝村/.test(message.text || "")), "SOT D-12 keyword branch reports source completion text");
assert(sotD12Game.dialog.debug.vmTrace.some((event) => event.action === "take" && Number(event.detail?.itemId) === 2586 && event.detail?.reason === "source-changeevent-message-delitem"), "SOT D-12 keyword branch takes item 2586 through NPC VM");
assert(sotD12Game.dialog.debug.vmTrace.some((event) => event.action === "give" && Number(event.detail?.itemId) === 2587 && event.detail?.reason === "source-changeevent-message-getitem"), "SOT D-12 keyword branch gives item 2587 through NPC VM");
let sotD12PassedGame = await api("/api/game/new", { name: "sainasu-sot-d12-passed-runtime-test" });
sotD12PassedGame.location = { mapId: "100", x: sotD12Npc.x + 1, y: sotD12Npc.y };
sotD12PassedGame.inventory.push({ id: 2587, name: "检查石", qty: 1 });
sotD12PassedGame = await api("/api/game/dialog", { game: sotD12PassedGame, npcId: sotD12Npc.id });
assertEqual(inventoryQty(sotD12PassedGame, 2587), 1, "SOT D-12 already-passed branch does not duplicate final checkpoint stone");
assert(sotD12PassedGame.dialog.messages.some((message) => /所有的检查点/.test(message.text || "") && /玛丽娜丝村/.test(message.text || "")), "SOT D-12 already-passed branch directs the player to source settlement");
let sotD12EarlyGame = await api("/api/game/new", { name: "sainasu-sot-d12-too-early-runtime-test" });
sotD12EarlyGame.location = { mapId: "100", x: sotD12Npc.x + 1, y: sotD12Npc.y };
sotD12EarlyGame.inventory.push({ id: 2585, name: "检查石", qty: 1 });
sotD12EarlyGame = await api("/api/game/dialog", { game: sotD12EarlyGame, npcId: sotD12Npc.id });
assertEqual(inventoryQty(sotD12EarlyGame, 2585), 1, "SOT D-12 too-early branch preserves tenth checkpoint stone");
assertEqual(inventoryQty(sotD12EarlyGame, 2587), 0, "SOT D-12 too-early branch does not grant final checkpoint stone");
assert(sotD12EarlyGame.dialog.messages.some((message) => /还太早了一点/.test(message.text || "") && /按照顺序通过/.test(message.text || "")), "SOT D-12 too-early branch uses source order text");

const sotDSettlementNpc = WORLD.maps["2000"]?.npcs.find((npc) => npc.id === "2000-88-116-7703");
if (!sotDSettlementNpc) throw new Error("missing Marinas SOT D-route settlement fixture");
assertEqual(sotDSettlementNpc.name, "检查员(D)", "SOT D-route settlement keeps source NPC name");
const sotDSettlementEvent = sotDSettlementNpc.scriptEvents?.find((event) => event.type === "MESSAGE" && event.condition === "LV>0&ITEM=2587");
const sotDRewardIds = sotDSettlementEvent?.getRandItems?.[0]?.ids || [];
assert(sotDSettlementEvent?.delItems?.some((item) => Number(item.id) === 2587), "SOT D settlement parses final checkpoint stone consumption");
assertEqual(sotDRewardIds.filter((id) => id === 20801).length, 2, "SOT D settlement preserves duplicate item 20801 reward weight");
assertEqual(sotDRewardIds.filter((id) => id === 68).length, 2, "SOT D settlement preserves duplicate item 68 reward weight");
assert(sotDSettlementEvent?.getPets?.some((spec) => [1797, 1796, 1795, 1794].every((id) => spec.enemyIds?.includes(id))), "SOT D settlement parses source random pet reward pool");
assert(/宠物数满了/.test(sotDSettlementEvent?.messages?.petFull || "") && /宠物弄少一点/.test(sotDSettlementEvent?.messages?.petFull || ""), "SOT D settlement splits inline source PetFullMsg from ItemFullMsg");
let sotDSettlementWrongRouteGame = await api("/api/game/new", { name: "marinas-sot-d-settlement-flag-gate-test" });
sotDSettlementWrongRouteGame.location = { mapId: "2000", x: sotDSettlementNpc.x + 1, y: sotDSettlementNpc.y };
setTestEventFlag(sotDSettlementWrongRouteGame, 17, "end");
sotDSettlementWrongRouteGame.inventory.push({ id: 2587, name: "检查石", qty: 1 });
sotDSettlementWrongRouteGame = await api("/api/game/dialog", { game: sotDSettlementWrongRouteGame, npcId: sotDSettlementNpc.id });
assertEqual(inventoryQty(sotDSettlementWrongRouteGame, 2587), 1, "SOT D settlement preserves final stone for a C-route participant");
assert(sotDSettlementWrongRouteGame.dialog.messages.some((message) => /D路线/.test(message.text || "") && /C路线/.test(message.text || "") && /萨姆吉尔/.test(message.text || "")), "SOT D settlement uses source wrong-route guidance");
let sotDPetFullGame = await api("/api/game/new", { name: "marinas-sot-d-settlement-pet-full-test" });
sotDPetFullGame.location = { mapId: "2000", x: sotDSettlementNpc.x + 1, y: sotDSettlementNpc.y };
sotDPetFullGame.inventory.push({ id: 2587, name: "检查石", qty: 1 });
const sotDPetFullFixture = { ...sotDPetFullGame.pets[0] };
while (sotDPetFullGame.pets.length < 5) sotDPetFullGame.pets.push({ ...sotDPetFullFixture });
sotDPetFullGame = await api("/api/game/dialog", { game: sotDPetFullGame, npcId: sotDSettlementNpc.id });
assertEqual(inventoryQty(sotDPetFullGame, 2587), 1, "SOT D pet-full preflight preserves final checkpoint stone");
assertEqual(sotDPetFullGame.pets.length, 5, "SOT D pet-full preflight does not add a partial reward pet");
assert(sotDPetFullGame.dialog.messages.some((message) => /宠物数满了/.test(message.text || "") && /宠物弄少一点/.test(message.text || "")), "SOT D pet-full preflight returns separately parsed source PetFullMsg");
assert(sotDPetFullGame.dialog.debug.vmTrace.some((event) => event.action === "quest" && event.status === "blocked" && event.detail?.reason === "pet-full"), "SOT D pet-full preflight is recorded before settlement mutations");
let sotDSettlementGame = await api("/api/game/new", { name: "marinas-sot-d-settlement-runtime-test" });
sotDSettlementGame.location = { mapId: "2000", x: sotDSettlementNpc.x + 1, y: sotDSettlementNpc.y };
sotDSettlementGame.inventory.push({ id: 2587, name: "检查石", qty: 1 });
const sotDSettlementPetCount = sotDSettlementGame.pets.length;
const originalRandomForSotDSettlement = Math.random;
try {
  Math.random = () => 0;
  sotDSettlementGame = await api("/api/game/dialog", { game: sotDSettlementGame, npcId: sotDSettlementNpc.id });
} finally {
  Math.random = originalRandomForSotDSettlement;
}
assertEqual(inventoryQty(sotDSettlementGame, 2587), 0, "SOT D settlement consumes final checkpoint stone");
assertEqual(inventoryQty(sotDSettlementGame, 20805), 1, "SOT D settlement grants one deterministic source random item fixture");
assertEqual(sotDSettlementGame.pets.length, sotDSettlementPetCount + 1, "SOT D settlement grants one source random pet");
assert(sotDSettlementGame.dialog.messages.some((message) => /辛苦你了/.test(message.text || "") && /SOT所有的检查点/.test(message.text || "")), "SOT D settlement reports source completion message");
assert(sotDSettlementGame.dialog.debug.vmTrace.some((event) => event.action === "take" && Number(event.detail?.itemId) === 2587 && event.detail?.reason === "source-changeevent-message-delitem"), "SOT D settlement consumes item 2587 through NPC VM");
assert(sotDSettlementGame.dialog.debug.vmTrace.some((event) => event.action === "give" && Number(event.detail?.itemId) === 20805 && event.detail?.reason === "source-changeevent-message-getitem"), "SOT D settlement grants random item through NPC VM");
assert(sotDSettlementGame.dialog.debug.vmTrace.some((event) => event.action === "givePet" && event.detail?.reason === "source-changeevent-getpet" && event.detail?.givenPets?.some((pet) => Number(pet.enemyId) === 1797)), "SOT D settlement grants random pet through NPC VM");
